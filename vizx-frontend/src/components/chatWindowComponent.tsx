...
const ChatbotWindow: React.FC = () => {
  ...
  const zohoCheckInterval = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  useEffect(() => {
    const checkZohoReady = () => {
      const zoho = (window as any).$zoho || (window as any).ZOHO;
      if (zoho?.salesiq) {
  ...
  useEffect(() => {
    ...
    if ((window as any).$zoho?.salesiq?.ready) {
      const originalReady = (window as any).$zoho.salesiq.ready;
      (window as any).$zoho.salesiq.ready = function () {
        if (typeof originalReady === "function") originalReady.call(this);
        handleZohoReady();
      };
    }
  ...
  const sendToZoho = (text: string) => {
    ...
    try {
      ...
      if (zoho?.salesiq?.visitor) {
        if (typeof zoho.salesiq.visitor.chatmessage === "function") {
          zoho.salesiq.visitor.chatmessage(text);
          messageSent = true;
        } else if (typeof zoho.salesiq.visitor.offlineMessage === "function") {
          zoho.salesiq.visitor.offlineMessage(text);
          messageSent = true;
        } else if (typeof zoho.salesiq.visitor.chat === "function") {
          zoho.salesiq.visitor.chat(text);
          messageSent = true;
        } else if (zoho.salesiq.chat?.start && typeof zoho.salesiq.chat.start === "function") {
  ...
  const openZohoChatWindow = () => {
    try {
      const zoho = (window as any).$zoho || (window as any).ZOHO;
      if (zoho?.salesiq) {
        if (zoho.salesiq.chatwindow?.open && typeof zoho.salesiq.chatwindow.open === "function") {
          zoho.salesiq.chatwindow.open();
        } else if (zoho.salesiq.floatwindow?.show && typeof zoho.salesiq.floatwindow.show === "function") {
  ...
  const startZohoChat = () => {
    try {
      const zoho = (window as any).$zoho || (window as any).ZOHO;
      if (zoho?.salesiq) {
        if (zoho.salesiq.chat?.start && typeof zoho.salesiq.chat.start === "function") {
          zoho.salesiq.chat.start();
    ...
    >
      ...
      {mode === "agent" && (
        <div
          className={`px-2 py-1 text-xs text-center ${
            isZohoReady ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"
          }`}
        >
      ...
      <MainContainer className={isMobile ? "h-[calc(100%-80px)]" : "py-12"}>
        <ChatContainer>
          <MessageList
            typingIndicator={
              mode === "ai" && isTyping ? <TypingIndicator content="Assistant is answering..." /> : null
            }
