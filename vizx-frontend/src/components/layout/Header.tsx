...
const Header: React.FC = () => {
  ...
  return (
    <header className="fixed inset-x-0 top-4 z-50 bg-transparent">
      {/* full-width rail that never overflows */}
      <div className="pointer-events-none w-full px-3 sm:px-4">
        <div className="pointer-events-auto w-full">
          {/* full-width pill with static ember effect */}
          <div
            className="
              ember-bar
              w-full
    ...
    ['--ember-edge' as any]: 0.7,
          ...
          >
            ...
            {/* Middle column: nav moved to the RIGHT */}
            <div className="hidden lg:block justify-self-end">
              <nav>
                <ul className="flex items-center justify-end gap-8 xl:gap-10 pr-0 md:pr-2">
                  ...
                  <li>
                    <Link to="/vizx-hi" className="py-2 text-gray-200 hover:text-orange-600 text-sm">
                      Vizx HI + AI
                    </Link>
                  </li>
                  <li>
                    <Link to="/why-kenya" className="py-2 text-gray-200 hover:text-orange-600 text-sm">
                      Why Kenya
                    </Link>
                  </li>
            ...
            {/* Right: CTA stays far right */}
            <div className="hidden lg:flex items-center justify-end ml-4 md:ml-6">
              <Link
                to="/contact"
                className="inline-flex items-center rounded-full bg-white text-black text-sm font-medium px-5 py-2 hover:bg-white/90 transition"
              >
                CONTACT US
              </Link>
            </div>
      ...
      {/* Mobile drawer … (unchanged placeholder) */}
      {mobileOpen && (
        <div className="fixed inset-0 z-40 flex">
          <div className="flex-1 bg-black/50" onClick={closeMobile} />
          <div className="w-72 bg-black p-4 overflow-y-auto">
            {/* … add your mobile nav content here if needed … */}
          </div>
        </div>
      )}
    </header>
                  ...
                  <li key={it.to}>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="opacity-0 group-hover/item:opacity-100 transition" aria-hidden="true">→</span>
                      </div>
