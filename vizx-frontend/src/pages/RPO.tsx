    ...
    <div className="bg-black text-white w-full font-sans">
      ...
      >
        ...
        <div className="relative z-10 px-4 text-center lg:text-left max-w-5xl">
          ...
          <motion.h2
            ...
            className="text-xl sm:text-2xl md:text-3xl text-gray-200 mb-6"
          >
          ...
          <motion.p
            ...
            className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-300 max-w-3xl leading-relaxed mx-auto lg:mx-0"
          >
            We specialize in connecting top talent with the right opportunities. Our expert team is dedicated to finding the perfect match for both employers and job seekers, ensuring success and growth for all. Discover your next career move or find your ideal candidate with us.
          </motion.p>
        </div>
      </section>
      ...
      <section className="px-4 md:px-8 py-12 md:py-16" data-aos="fade-up">
        <div className="max-w-5xl mx-auto">
          <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-orange-500 text-center mb-8">
            Areas Where We Specialize
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {['Recruitment', 'Quality Control', 'Payroll', 'Staffing'].map((item) => (
              <div key={item} className="bg-gray-800 p-6 rounded-lg flex items-center justify-center">
      ...
      <section className="px-4 md:px-8 py-12 md:py-16 bg-gray-900" data-aos="fade-up">
        <div className="max-w-5xl mx-auto">
          <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-orange-500 text-center mb-6">
            Why Is VizX Ideal as Your RPO Partner
          </h3>
          <p className="text-base md:text-lg text-gray-300 text-center mb-8 leading-relaxed">
            We understand the know-hows of the business
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              { title: 'Experience', text: 'With over 20 years in the RPO industry, our comprehensive Recruitment, Quality Compliance and Payroll service ensure that your organization has access to the best talent while maintaining streamlined and efficient processes.' },
              { title: 'Excellence', text: 'Our rigorous Quality Compliance process ensures that you experience peace of mind in knowing your staff are well in line with International Industry Standards of their respective disciplines.' },
              { title: 'Updated Seamless Services', text: 'We ensure your staff receives seamless Payroll services ensuring hours are well compensated as per State and Federal Laws, benefits remittances and compensations are performed. Also ensuring returns and documents are handled electronically and timely.' },
              { title: 'Tailored Approach', text: 'We present a tailored approach that meets your specific recruitment needs, resulting in higher productivity and a successful workforce.' },
            ].map((item, i) => (
              <div key={i} className="bg-black p-6 md:p-8 rounded-lg">
                <h4 className="text-xl md:text-2xl font-bold text-orange-500 mb-2">{item.title}</h4>
                <p className="text-sm md:text-base text-gray-200 leading-relaxed">{item.text}</p>
              </div>
            ))}
          </div>
          <div className="text-center mt-8">
            <a href="/contact" className="inline-block bg-orange-500 hover:bg-orange-600 text-black font-semibold py-3 px-6 rounded-md transition">
              Get Started Today
            </a>
          </div>
        </div>
      ...
      <section className="px-4 md:px-8 py-12 md:py-16" data-aos="fade-up">
        <div className="max-w-5xl mx-auto space-y-12">
          <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-orange-500 text-center">
            Benefits You Gain From VizX
          </h3>
          {
            [
              ...
              <div
                key={idx}
                className={`grid grid-cols-1 md:grid-cols-2 gap-8 bg-gray-800 p-6 rounded-lg items-center ${item.reverse ? 'md:flex-row-reverse' : ''}`}
              >
                <div className="w-full">
                  <img src={item.img} alt={item.title} className="w-full h-auto rounded-md object-cover" />
                </div>
                <div className="w-full space-y-2">
                  <h4 className="text-xl md:text-2xl font-bold text-orange-500">{item.title}</h4>
                  {item.points.map((pt, i2) => (
                    <p key={i2} className="text-sm md:text-base text-gray-200 leading-relaxed">{pt}</p>
                  ))}
                </div>
              </div>
            ))
      ...
      <section className="relative overflow-hidden py-16 px-4 md:px-8 text-white" data-aos="fade-up">
        ...
        <div className="relative z-20 max-w-6xl mx-auto text-center space-y-6">
          <h2 className="text-3xl md:text-4xl font-bold text-orange-500">VIZX Pricing Advantage</h2>
          ...
          <div className="text-gray-300 max-w-4xl mx-auto leading-relaxed">
            <p className="mb-4">
          ...
          <button className="bg-orange-500 hover:bg-orange-600 text-black font-semibold px-6 py-3 rounded-md transition">
            Get Started Today
          </button>
        </div>
      </section>
            ...
            {[
              ...
              <li key={idx}>
                <button
                  ...
                  className="w-full text-left bg-gray-800 p-4 rounded-md focus:outline-none flex justify-between items-center"
                >
                  <span className="font-bold text-orange-500">{item.question}</span>
